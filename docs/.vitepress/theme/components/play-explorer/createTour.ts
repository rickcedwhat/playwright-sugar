  ...
  switch (phase) {
    ...
    case 'outcomes':
      return {
        ...
        hint: 'The mock shows the real locator for your pick: success headline, failure toast, or a 3s timeout wait. `Outcomes.timeout()` wins if neither beats the attempt timeout option.',
      };
    case 'director':
      return {
...
export function phaseBullets(phase: CreatePhaseId): string[] {
  switch (phase) {
    ...
    case 'outcomes':
      return [
        'Outcomes.success watches for the detail headline; Outcomes.failure watches Sonner toasts; timeout resolves if neither beats the 3s timeout option.',

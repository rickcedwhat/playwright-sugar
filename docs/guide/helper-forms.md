...
2. **Lite (copy-paste)** — under [`/snippets`](https://github.com/rickcedwhat/playwright-sugar/tree/main/snippets). **Generated** from the robust source by stripping `sugar-full-only` regions (`pnpm run snippets:generate`). Same core algorithm; no hand-maintained duplicate. Snippets import **`@playwright/test` only** — never other sugar helpers.
